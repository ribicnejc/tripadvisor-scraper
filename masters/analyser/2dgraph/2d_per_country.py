...
def graph(year_from, year_to, country, color):
    ...
    dataset = {'Zemljepisna dolžina': x_arr, 'Zemljepisna širina': y_arr}
    dset = pd.DataFrame(data=dataset)
    sns.set_theme()
    # Custom the inside plot: options are: “scatter” | “reg” | “resid” | “kde” | “hex”
    sns.jointplot(x=dset['Zemljepisna dolžina'], y=dset['Zemljepisna širina'], kind='scatter', color=color)
    plt.savefig(f'{country}-{year_from}-scatter.png'.format(country=country, year_from=year_from))
    sns.jointplot(x=dset['Zemljepisna dolžina'], y=dset['Zemljepisna širina'], kind='kde', color=color)
